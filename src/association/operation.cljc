(ns association.operation
  "OperationActor -- one association operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (AssocOps-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Association Governance Governor
  ...
  One graph run = one association operation (intake -> advise ->
  ...
  decision to a human operator (the association officer). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:certification/issue`/`:discipline/finalize` ALWAYS reach this node
  when the governor is clean -- see `association.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [association.assocopsllm :as assocopsllm]
            [association.governor :as governor]
...
(defn build
  ...
  opts:
    :advisor      -- an `association.assocopsllm/Advisor` (default: mock-advisor)
